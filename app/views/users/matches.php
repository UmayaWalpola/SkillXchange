                    ...
                    <?php foreach ($data['pendingRequests'] as $request): ?>
                        <div class="request-card">
                            ...
                            <div class="request-actions">
                                <button class="btn-accept" onclick="handleRequest(<?= $request['exchange_id']; ?>, 'accept')">Accept</button>
                                <button class="btn-reject" onclick="handleRequest(<?= $request['exchange_id']; ?>, 'reject')">Reject</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                        ...
                        <optgroup label="Skills I Teach">
                            <?php if (!empty($data['userSkills']['teaches'])): ?>
                                <?php foreach ($data['userSkills']['teaches'] as $skill): ?>
                                    <option value="<?= htmlspecialchars($skill['name']); ?>"><?= htmlspecialchars($skill['display']); ?></option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </optgroup>
            ...
            <div class="match-tier-section" data-tier="mutual">
                ...
                <div class="matches-grid">
                    <?php foreach ($data['mutual'] as $match): ?>
                        <?php
                            // Mutual: current user can both teach AND learn
                            // Pass both skill names so chat modal can offer role choice
                            $iTeachSkill  = !empty($match['i_teach'])    ? $match['i_teach'][0]['name']    : '';
                            $theyTeachSkill = !empty($match['they_teach']) ? $match['they_teach'][0]['name'] : '';
                        ?>
                        <div class="match-card mutual-match"
                             data-tier="mutual"
                             data-match-type="mutual"
                            ...
                            <div class="match-header">
                                <div class="match-avatar"><?= htmlspecialchars($match['avatar']); ?></div>
                                <div class="match-basic-info">
                                    <h3 class="match-name" onclick="viewProfile(<?= $match['id']; ?>)"><?= htmlspecialchars($match['name']); ?></h3>
                            ...
                            <div class="match-skills-section">
                                ...
                                <div class="skill-direction">
                                    <div class="direction-header"><span class="direction-label">You Teach</span></div>
                                ...
                                <div class="skill-direction">
                                    <div class="direction-header"><span class="direction-label">You Learn</span></div>
                            ...
                            <div class="match-footer">
                                <span class="compatibility-score"><?= $match['total_skills']; ?> skill<?= $match['total_skills'] > 1 ? 's' : ''; ?> matched</span>
                                ...
                                <?php elseif (isset($match['connection_status']) && $match['connection_status'] === 'connected'): ?>
                                    <div class="connected-actions">
                                        ...
                                        <button type="button" class="btn-chat"
                                            onclick="openSkillChat(
                                                <?= (int)$match['id']; ?>,
                                                'mutual',
                                                '<?= htmlspecialchars($iTeachSkill, ENT_QUOTES); ?>',
                                                '<?= htmlspecialchars($theyTeachSkill, ENT_QUOTES); ?>'
                                            )">
                                            Go to Chat
                                        </button>
                                    </div>
            ...
            <div class="match-tier-section" data-tier="multi">
                ...
                <div class="matches-grid">
                    <?php foreach ($data['multi'] as $match): ?>
                        <?php
                            // Multi but not mutual: all skills go same direction
                            $iTeachSkill    = !empty($match['i_teach'])    ? $match['i_teach'][0]['name']    : '';
                            $theyTeachSkill = !empty($match['they_teach']) ? $match['they_teach'][0]['name'] : '';
                            // For non-mutual multi: current user is either purely teacher or purely learner
                            $multiDirection = $iTeachSkill ? 'teacher' : 'learner';
                            $multiSkill     = $iTeachSkill ?: $theyTeachSkill;
                        ?>
                        <div class="match-card multi-match"
                             data-tier="multi"
                             data-match-type="multi"
                             data-skills="<?= htmlspecialchars(json_encode(array_merge(
                                 array_column($match['i_teach'], 'name'),
                                 array_column($match['they_teach'], 'name')
                             ))); ?>">
                            ...
                            <div class="match-footer">
                                <span class="compatibility-score"><?= $match['total_skills']; ?> skills matched</span>
                                <?php if (isset($match['connection_status']) && $match['connection_status'] === 'pending'): ?>
                                    <span class="badge badge-warning">⏳ Pending</span>
                                <?php elseif (isset($match['connection_status']) && $match['connection_status'] === 'connected'): ?>
                                    <div class="connected-actions">
                                        ...
                                        <button type="button" class="btn-chat"
                                            onclick="openSkillChat(
                                                <?= (int)$match['id']; ?>,
                                                'multi',
                                                '<?= htmlspecialchars($multiSkill, ENT_QUOTES); ?>',
                                                '<?= htmlspecialchars($multiDirection, ENT_QUOTES); ?>'
                                            )">
                                            Go to Chat
                                        </button>
                                    </div>
            ...
            <div class="match-tier-section" data-tier="single">
                ...
                <div class="matches-grid">
                    <?php foreach ($data['single'] as $match): ?>
                        <?php
                            // Single: strictly one direction
                            $singleSkill     = !empty($match['i_teach']) ? $match['i_teach'][0]['name'] : (!empty($match['they_teach']) ? $match['they_teach'][0]['name'] : '');
                            $singleDirection = !empty($match['i_teach']) ? 'teacher' : 'learner';
                        ?>
                        <div class="match-card single-match"
                             data-tier="single"
                             data-match-type="single"
                             data-skills="<?= htmlspecialchars(json_encode(array_merge(
                                 array_column($match['i_teach'], 'name'),
                                 array_column($match['they_teach'], 'name')
                             ))); ?>">
                            ...
                            <div class="match-footer">
                                <span class="compatibility-score">1 skill matched</span>
                                <?php if (isset($match['connection_status']) && $match['connection_status'] === 'pending'): ?>
                                    <span class="badge badge-warning">⏳ Pending</span>
                                <?php elseif (isset($match['connection_status']) && $match['connection_status'] === 'connected'): ?>
                                    <div class="connected-actions">
                                        ...
                                        <button type="button" class="btn-chat"
                                            onclick="openSkillChat(
                                                <?= (int)$match['id']; ?>,
                                                'single',
                                                '<?= htmlspecialchars($singleSkill, ENT_QUOTES); ?>',
                                                '<?= htmlspecialchars($singleDirection, ENT_QUOTES); ?>'
                                            )">
                                            Go to Chat
                                        </button>
                                    </div>
