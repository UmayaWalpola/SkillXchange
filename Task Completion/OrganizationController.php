...
class OrganizationController extends Controller {
    ...
    public function createProject() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $projectType = strtolower(trim((string)($_POST['project_type'] ?? 'commercial')));
            ...
            if (empty($_POST['max_members']) || $maxMembers < 1) {
                $errors['max_members'] = 'Max members must be at least 1';
            } elseif ($projectType === 'commercial' && $maxMembers < 6) {
                $errors['max_members'] = 'Commercial projects require a minimum of 6 members';
            }
    ...
    public function editProject($projectId = null) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $projectType = strtolower(trim((string)($_POST['project_type'] ?? ($project->project_type ?? 'commercial'))));
            $rawRequiredSkills = trim((string)($_POST['required_skills'] ?? ''));
            $startDate = trim((string)($_POST['start_date'] ?? ''));
            $endDate = trim((string)($_POST['end_date'] ?? ''));
            ...
            if (empty($_POST['max_members']) || $maxMembers < 1) {
                ...
                $errors['max_members'] = 'Commercial projects require min of 6 members';
            }
